...
public class FixMessageHandler : IFixMessageHandler
{
    ...
    private readonly IMarketDataObserver? _signalRTickObserver;
    ...
    public FixMessageHandler(
        ...
        IMarketDataObserver? signalRTickObserver = null)
    {
        ...
        _signalRTickObserver = signalRTickObserver;
    }
    ...
    public void Handle(DtoMarketData tick)
    {
        ...
        // Redis + in-memory + SignalR: sadece limit içindeki (doğru) veri; ihlalde önceki doğru değer korunur
        if (!breach)    // İhlal yok, tick'i Redis, in-memory ve SignalR observer'lara gönder
        {
            _redisStoreTickObserver.OnTick(tick);
            _inMemoryLastPriceObserver.OnTick(tick);
            _signalRTickObserver?.OnTick(tick);
        }
    }
}
